...
<sub>ARQUIVO CENTRAL · 9 REGISTROS · ACESSO CONCEDIDO</sub>
...
### 🎛️ [IROS — Overlay](https://github.com/olimpio00/Overlay) ![EM OPERAÇÃO](https://img.shields.io/badge/EM%20OPERA%C3%87%C3%83O-5c1113?style=flat-square)
...
`ARMAMENTO` · `Go` `JavaScript` `HTML/CSS` — backend em Go servindo templates e assets

> Arquitetura em camadas (`cmd/`, `core/`, `meta/`), estado de sessão em tempo real e configuração via `.env`.

<sub>`M-002` &nbsp; `SETOR` Rede de transmissão &nbsp; `CLASSIFICAÇÃO` AGPL-3.0 · solo</sub>

---

### 🎴 [GachArena](https://github.com/olimpio00/GachArena) ![EM OPERAÇÃO](https://img.shields.io/badge/EM%20OPERA%C3%87%C3%83O-5c1113?style=flat-square)
...
`ARMAMENTO` · `TypeScript` `Angular` `Node.js` `GitHub Actions` — monorepo `frontend/` + `backend/` com pipeline de CI

> Modelagem de probabilidade e raridade — a parte difícil é garantir que o drop rate seja justo *e* auditável.

<sub>`M-003` &nbsp; `SETOR` Bordas Exteriores &nbsp; `CLASSIFICAÇÃO` Solo · 36 de 36 commits</sub>

---

### 🛒 [ProdutorStore](https://github.com/olimpio00/produtor-store) ![ARQUIVADA](https://img.shields.io/badge/ARQUIVADA-5c1113?style=flat-square)

E-commerce em **Angular 17** com catálogo de produtos, consumindo API mockada via `json-server` e proxy de desenvolvimento.

`ARMAMENTO` · `Angular 17` `TypeScript` `JSON Server`

<sub>`M-004` &nbsp; `SETOR` Rotas comerciais &nbsp; `CLASSIFICAÇÃO` Solo</sub>

---

### ⚛️ [Refactory-React](https://github.com/olimpio00/Refactory-React) ![CONCLUÍDA](https://img.shields.io/badge/CONCLU%C3%8DDA-5c1113?style=flat-square)

Estudo prático de **arquitetura de frontend**: refatoração de um `ProductPage` monolítico em uma estrutura escalável.

`ARMAMENTO` · `React` `TypeScript` `Vite`
...
<sub>`M-005` &nbsp; `SETOR` Treinamento &nbsp; `CLASSIFICAÇÃO` Estudo de arquitetura</sub>

---

### 🎵 [Player Music](https://github.com/olimpio00/player-music) ![EM OPERAÇÃO](https://img.shields.io/badge/EM%20OPERA%C3%87%C3%83O-5c1113?style=flat-square)
...
`ARMAMENTO` · `Python 3.11` `tkinter` `Pygame` `Mutagen` `PyWin32` `PyInstaller`

> Empacotado como `.exe` via PyInstaller. Integração com as media keys do Windows foi a parte mais interessante.

<sub>`M-006` &nbsp; `SETOR` Sinais &nbsp; `CLASSIFICAÇÃO` MIT · desktop</sub>

---

### ⬇️ [Spotify → YouTube Downloader](https://github.com/olimpio00/Spotify-YouTube-Downloader) ![EM OPERAÇÃO](https://img.shields.io/badge/EM%20OPERA%C3%87%C3%83O-5c1113?style=flat-square)
...
`ARMAMENTO` · `Python` `CustomTkinter` `yt-dlp` `Spotipy` `FFmpeg`

> Integração com a Spotify Web API + pipeline de conversão em FFmpeg, com progresso em tempo real na UI.

<sub>`M-007` &nbsp; `SETOR` Contrabando &nbsp; `CLASSIFICAÇÃO` Solo · desktop</sub>

---

### 🗂️ [Star Wars Fan Wiki](https://github.com/olimpio00/Star-Wars-Fan-Wiki) · [🌐 ao vivo](https://star-wars-fan-wiki.onrender.com) ![AO VIVO](https://img.shields.io/badge/AO%20VIVO-5c1113?style=flat-square)
...
`ARMAMENTO` · `Python` `NiceGUI` `Supabase` (auth + storage + database)

<sub>`M-008` &nbsp; `SETOR` Arquivos &nbsp; `CLASSIFICAÇÃO` MIT · Render</sub>

---

### 🕵️ [Andor Missions](https://github.com/olimpio00/Andor-Missions) ![EM OPERAÇÃO](https://img.shields.io/badge/EM%20OPERA%C3%87%C3%83O-5c1113?style=flat-square)
...
`ARMAMENTO` · `Python` `Flet` `Supabase` — deploy declarativo via `render.yaml`

<sub>`M-009` &nbsp; `SETOR` Inteligência rebelde &nbsp; `CLASSIFICAÇÃO` Solo · 10 de 10 commits</sub>
